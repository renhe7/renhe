...
```{r,echo=TRUE,eval=TRUE}
historic_state_data_url <- "https://api.covidactnow.org/v2/states.timeseries.json?apiKey=b7246133c9fd4c55874f90933e53624f"
...
```{r,echo=TRUE,eval=TRUE}
time_series_cases <- list()

time_series_cases$Alaska <- time_series %>%
  filter(state == "AK") %>%
  select(date, cases)
...
time_series_transmission[seq(1,1300,by=100),]%>%
   pivot_longer(cols=Alaska:District_of_Columbia, 
                names_to="Countries",
                values_to="Transmission") %>%
...
fig5<- ggplot(data_to_plot, aes(x=Date_District_of_Columbia,y=Cases_District_of_Columbia)) + 
  geom_point() + labs(x="Date",y="Cases", title="District of Columbia") +
  theme_bw()
